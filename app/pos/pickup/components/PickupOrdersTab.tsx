...
export default function PickupOrdersTab() {
  ...
  const previousOrdersRef = useRef<ManagerOrder[]>([]);
  ...
  const readyUnpaid = readyOrders.filter(
    (o) => o.paymentType !== "card"
  );

  const readyPaid = readyOrders.filter(
    (o) => o.paymentType === "card"
  );
  ...
  return (
    <div className="min-h-screen px-8 py-2 bg-violet-100 flex flex-col gap-4">
 ...
 {readyPaid.map((order) => (
          ...
          {readyPaid.length === 0 && (
            <p className="text-xs italic text-white">~ No paid ready orders.</p>
          )}
        </div>
      </div>
